...
module old_fpnew_fma_multi #(
  ...
  input  old_fpnew_pkg::fp_format_e       src_fmt_i, // format of the multiplicands
  input  old_fpnew_pkg::fp_format_e       dst_fmt_i, // format of the addend and result
  ...
  localparam int unsigned EXP_WIDTH = old_fpnew_pkg::maximum(SUPER_EXP_BITS + 2, LZC_RESULT_WIDTH);
...
// verilator lint_off BLKANDNBLK
  ...
  old_fpnew_pkg::fp_info_t [NUM_FORMATS-1:0][2:0] info_q;
  ...
  for (genvar fmt = 0; fmt < int'(NUM_FORMATS); fmt++) begin : fmt_init_inputs
    ...
    if (FP_FMT_CONFIG[fmt]) begin : active_format
      ...
      assign info_q[fmt]                 = '{default: old_fpnew_pkg::DONT_CARE}; // format disabled
  ...
  old_fpnew_pkg::fp_info_t info_a,    info_b,    info_c;
  ...
  always_comb begin : op_select
    ...
    unique case (inp_pipe_op_q[NUM_INP_REGS])
      old_fpnew_pkg::FMADD:  ; // do nothing
      old_fpnew_pkg::FNMSUB: operand_a.sign = ~operand_a.sign; // invert sign of product
      old_fpnew_pkg::ADD: begin // Set multiplicand to +1
        operand_a = '{sign: 1'b0, exponent: old_fpnew_pkg::bias(src_fmt_q), mantissa: '0};
        info_a    = '{is_normal: 1'b1, is_boxed: 1'b1, default: 1'b0}; //normal, boxed value.
      end
      old_fpnew_pkg::MUL: begin // Set addend to +0 or -0, depending whether the rounding mode is RDN
        if (inp_pipe_rnd_mode_q[NUM_INP_REGS] == old_fpnew_pkg::RDN)
      ...
      default: begin // propagate don't cares
        operand_a  = '{default: old_fpnew_pkg::DONT_CARE};
        operand_b  = '{default: old_fpnew_pkg::DONT_CARE};
        operand_c  = '{default: old_fpnew_pkg::DONT_CARE};
        info_a     = '{default: old_fpnew_pkg::DONT_CARE};
        info_b     = '{default: old_fpnew_pkg::DONT_CARE};
        info_c     = '{default: old_fpnew_pkg::DONT_CARE};
      end
    endcase
  end
  ...
  old_fpnew_pkg::status_t special_status;
  logic               result_is_special;

  logic [NUM_FORMATS-1:0][WIDTH-1:0]    fmt_special_result;
  old_fpnew_pkg::status_t [NUM_FORMATS-1:0] fmt_special_status;
  ...
  assign exponent_product = (info_a.is_zero || info_b.is_zero) // in case the product is zero, set minimum exp.
                            ? 32'd2 - signed'(old_fpnew_pkg::bias(dst_fmt_q))
                            : signed'(exponent_a + info_a.is_subnormal
                                      + exponent_b + info_b.is_subnormal
                                      - 2*signed'(old_fpnew_pkg::bias(src_fmt_q))
                                      + signed'(old_fpnew_pkg::bias(dst_fmt_q))); // rebias for dst fmt
  ...
  old_fpnew_pkg::status_t            special_status_q;
  // verilator lint_off BLKANDNBLK
  ...
  old_fpnew_pkg::status_t    [NUM_MID_REGS:0]                         mid_pipe_spec_stat_q;
  ...
  assign of_before_round = final_exponent >= 2**(old_fpnew_pkg::exp_bits(dst_fmt_q2))-1; // infinity exponent is all ones
  ...
  for (genvar fmt = 0; fmt < int'(NUM_FORMATS); fmt++) begin : gen_res_assemble
    ...
    if (FP_FMT_CONFIG[fmt]) begin : active_format
      ...
      assign fmt_round_sticky_bits[fmt] = '{default: old_fpnew_pkg::DONT_CARE};
    end
  end
  ...
  old_fpnew_pkg::status_t   regular_status;
