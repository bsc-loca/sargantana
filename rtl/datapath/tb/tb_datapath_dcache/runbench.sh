...
vlog +acc=rn +incdir+ $INCLUDES/riscv_pkg.sv $INCLUDES/drac_pkg.sv $DRAC_FOLDER_RTL/register.sv\
 $IF_STAGE/if_stage.sv $ID_STAGE/decoder.sv $ID_STAGE/immediate.sv $ID_STAGE/free_list.sv $ID_STAGE/rename_table.sv $RR_STAGE/regfile.sv\
 $EXE_STAGE/exe_stage.sv $EXE_STAGE/alu.sv $EXE_STAGE/mem_unit.sv $EXE_STAGE/load_store_queue.sv $EXE_STAGE/mul_unit.sv $EXE_STAGE/div_unit.sv $EXE_STAGE/div_8bits.sv \
 $EXE_STAGE/branch_unit.sv $EXE_STAGE/score_board.sv $DCACHE/dcache_interface.sv $CONTROL/control_unit.sv $WB_STAGE/graduation_list.sv\
